...
int Transport::push(Message* m)
{
    ...
    DEBUG(cout << "To " << inet_ntoa(g_dstAddr.sin_addr) << endl);
    DEBUG(cout << "Sent Msg: " << m << endl);
    DEBUG(m->printBuffer((32)));
 ...
 */
int Transport::pop(vector<Message>& m, int numReqMsg, int& numRetMsg)
{
   uint8_t buffer[MAX_MESSAGE_SIZE];    // receive buffer TODO: Can't declare this every time too expensive
   ...
   int fromlen;                         // Length of sender's address.
    ...
    DEBUG(cout << "waiting on port " << this->n_srcPort << endl);
   ...
   for(int i = 0; i < numReqMsg; i++)
   {
       ...
       if (recvlen > 0) {
           ...
           m.push_back(Message(i,0,recvlen, buffer));
